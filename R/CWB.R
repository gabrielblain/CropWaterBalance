...
<<<<<<< HEAD
  ...
  if(D[1,1] > TAW[1,1]){D[1,1] <- TAW[1,1]}
    if (D[1,1] >= dmad[1,1]) {recom[1,1]=paste("Yes. Irrigate",round(D[1,1],3),"mm")}
    else {recom[1,1]=c("No")}
 if (D[1,1] > dmad[1,1]) {Ks[1,1] <- ((TAW[1,1]-D[1,1])/((1-MAD[1,1])*TAW[1,1]))}
    ...
    if (D[i,1] >= dmad[i,1]) {recom[i,1]=paste("Yes. Irrigate",round(D[i,1],3),"mm")}
    else {recom[i,1]=c("No")}
...
=======
  ...
  for (i in 2:n) {
    ...
    if (D[i, 1] < 0) {
      D[i, 1] <- 0
>>>>>>> 0909402f80faa6d04c5178cc860fa9aa24dda501
    }
