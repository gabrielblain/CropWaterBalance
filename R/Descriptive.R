...
Descriptive <- function(Sample){
  ...
  n <- length(Sample)
  if (is.numeric(Sample) == FALSE ||
      length(Sample) < 10)
      {
    stop("Sample must be a numerical single-column variable with at least 10 records.")
  } else if (length(Sample[Sample<0]) > 0) {
    stop("Negative or missing data in the sample is not allowed.")} else{
    Desc <- as.matrix(c(n,mean(Sample), median(Sample),
              sd(Sample),sd(Sample)/sqrt(n),
